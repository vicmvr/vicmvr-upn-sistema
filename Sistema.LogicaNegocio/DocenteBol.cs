...
namespace Sistema.LogicaNegocio
{
    public class DocenteBol
    {
        ...
        public EDocente TraerPorId(int idDocente)
        {
            stringBuilder.Clear();

            if (idDocente == 0) stringBuilder.Append("Por favor proporcione un valor de Id valido");

            if (stringBuilder.Length == 0)
            {
                return _docenteDal.GetByid(idDocente);
            }
            return null;
        }
        ...
        public void Eliminar(int idDocente)
        {
            ...
            if (stringBuilder.Length == 0)
            {
                _docenteDal.Delete(idDocente);
            }
        }

        private bool Validardocente(EDocente docente)
        {
            stringBuilder.Clear();
